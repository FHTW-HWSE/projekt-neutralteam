...
char *roomToString(Room *room) {
    char *roomString = (char*)malloc(sizeof(char) * BIG_MAGIC_NUMBER);
    ...
    for (int i = 0; i < room->rows; i++) {
        for (int j = 0; j < room->cols; j++) {
            if(strlen(room->seats[i*room->cols+j]) > 0) {
                char *seatString = (char*)malloc(sizeof(char) * BIG_MAGIC_NUMBER);
...
char *getRoomLayoutString(Room *room) {
    char *roomLayoutString = (char*)malloc(sizeof(char) * BIG_MAGIC_NUMBER);
    int currentIndex = 0;
    for (int i = 0; i < room->rows; i++) {
        for (int j = 0; j < room->cols; j++) {
